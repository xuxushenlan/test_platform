...
def add_project(request):
    ...
    if request.method == "POST":
        ...
        if status is None:
            ...
            return common.response_failed(message="状态类型错误")

        req = ProjectDao.get_object_by_name(name)
        if req is None:
            return common.response_failed(message="项目查询错误")
        elif len(req) != 0:
            return common.response_failed(message="项目已经存在")

        req = ProjectDao.create(name, describe, status)
        if req is None:
            return common.response_failed(message="创建项目失败")
...
def update_project(request):
    ...
    if request.method == "POST":
        ...
        if req is None:
            return common.response_failed(message="更新项目失败")

        return common.response_succeed(message="更新成功")
    else:
...
def get_projects(request):
    ...
    if request.method == "GET":
        ...
        if req is None:
            return common.response_failed(message="查询失败")

        return common.response_succeed(message="查询成功", data=req)
    else:
        return common.response_failed(message="请求方法错误")
...
def delete_project(request):
    ...
    if request.method == "POST":
        ...
        if id_ is None:
            return common.response_failed(message="id不能为空")

        ret = ProjectDao.delete_by_id(id_)
        if ret is None:
            return common.response_failed(message="删除失败")
